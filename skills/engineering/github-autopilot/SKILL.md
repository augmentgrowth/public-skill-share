...
- **A clean worktree is not an idle worktree, and mtime is a weak proxy for liveness.** A session merely between edits leaves no dirty files and may already be merged, so a live workspace and an abandoned one are indistinguishable by state alone. Worse, a session parked through a meeting or a day of parallel work is untouched but alive. Prefer a real signal (a process whose cwd is inside the worktree); otherwise use a wide age window — 7 days, or 30 for harness-managed session roots. Size these by cost asymmetry, not tidiness: deleting a live workspace breaks running work, while a dead worktree surviving one more sweep costs nothing.

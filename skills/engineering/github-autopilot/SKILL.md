...
- **A clean worktree is not an idle worktree.** A session that is merely between edits leaves no dirty files, and its branch may already be merged — so a live workspace and an abandoned one are indistinguishable by state alone. Check the worktree directory's modification time before removing it; anything touched in the last hour belongs to a running session.
